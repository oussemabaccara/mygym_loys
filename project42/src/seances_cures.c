...
void afficher_seance(GtkWidget *liste)	
{
    ...
    strcpy(anneee,s.date_seance.annee);
	strcpy(moiss,s.date_seance.mois);
	strcpy(jourr,s.date_seance.jour);
	...
	{ f = fopen("seances_cures.txt", "a+");
              while(fscanf(f,"%s %s %s %s %s\n", type,jourr,moiss,anneee,periode)!=EOF)
		{
	gtk_list_store_append (store, &iter);
	gtk_list_store_set (store, &iter,TYPE, type,JOUR,jourr,MOIS,moiss,ANNEE,anneee,PERIODE,periode ,-1); 
		}
		fclose(f);
